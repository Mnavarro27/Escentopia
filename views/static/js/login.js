document.addEventListener("DOMContentLoaded", () => {
  ...
  async function solicitarCodigo2FA(username) {
    try {
      ...
      let response = await fetch("/api/autenticacion/solicitar-2fa", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        ...
        response = await fetch("/solicitar-2fa", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ username }),
        })
      ...
      if (response.ok) {
        ...
        errorMessage.textContent = data.error || "Error al solicitar código 2FA"
        errorMessage.style.display = "block"
  ...
  function mostrarTiempoBloqueo(bloqueadoHasta) {
    ...
    function actualizarTiempo() {
      ...
      if (tiempoRestante > 0) {
        ...
        bloqueoMensaje.textContent = `Cuenta bloqueada. Intenta nuevamente en ${minutos}m ${segundos}s.`
